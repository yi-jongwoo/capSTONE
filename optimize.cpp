...
inline ll pre_objective(int n,vector<pair<int,int>>&edges,
...
void nonconvexed_decrossing(int n,vector<int>* arr,int w,int h,int* out_x,int* out_y){
	...
	for(int t=0;t<100;t++){
		for(int v=0;v<n;v++){
			ll z=pre_objective(n,edges,out_x,out_y,v,adjs[v]);
			int x=out_x[v]; 
			...
			ll nz=pre_objective(n,edges,out_x,out_y,v,adjs[v]);
			if(z<nz){
				out_x[v]=x;
				out_y[v]=y;
			}
	...
	#define  partial_objective pre_objective
